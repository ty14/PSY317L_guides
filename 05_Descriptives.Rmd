...
In the sections below, we are going to visually demonstrate this.  Hopefully this helps to show you that dividing by $n-1$ works. Don't worry too much about any code here, this is just to show you what's going on.
...
Let's assume the following data distribution is our population, we'll call it `pop`. The following code creates a population of 10000 numbers drawn from a random normal distribution (see section xxx.xxx) with a population mean of 8 and population standard deviation of 2. Because we're randomly drawing numbers to make our population, the final population won't have a mean and standard deviation that are precisely 8 and 2, but we can calculate what they turn out to be:
...
Here our sample mean $\overline{x}=7.88$ which is close, but a bit below $\mu=7.99$.  
...
Again our value of $\overline{x}=7.69$ is below $\mu=7.99$.
...
This time our value of $\overline{x}=8.58$ is higher than $\mu=7.99$.
...
Note: you don't need to know how this code works!
...
It turns out that the average sample mean that we collect using the formula $\overline{x} = \frac{\Sigma{x}}{n}$ is 7.99 which is the same as the population mean $\mu$. What this means is that this formula is perfectly fine to use to **estimate** the population mean. It is what we call an **unbiased estimator**.  Over the long run, it gives us a very good estimate of the population mean $\mu$.  Here is a histogram of our sample means from our 20,000 samples:
...
As a side note - what we just did in the exercise above was to calculate a *sampling distribution of sample means* - something we'll discuss much more in chapter xxx. 
...
With the first sample, both estimates are higher than $\sigma=2.02$, with the sample standard deviation being even higher.
...
With the second sample of 10, both estimates are lower than $\sigma=2.02$, with the sample standard deviation being closer to 2.02.
