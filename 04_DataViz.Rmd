...
Boxplots are a very useful way of summarizing the distribution of data. The image below summarizes what each line in the boxplot represents. For more details on all of these descriptive measures see section xxx.xxx:
...
To do this, instead of putting `x=strain` inside of `aes()` we put `x = reorder(strain, strain, median)` inside instead. This is a bit of a mouthful. To break it down, it's saying plot strain on the x-axis, but reorder the groups based on the median of the strain column (that's the second 'strain' in the code).


```{r, fig.width = 5, fig.height = 3, warning=FALSE, message=FALSE}
ggplot(wheels1, aes(x = reorder(strain, strain, median), y = day4)) + 
...
We need to turn this to **long data** to be able to make the stacked boxplot graph. See section xxx.xxx for more on how to switch between wide and long data formats:
...
ggplot(wheels2.long, aes(x = strain, y = value, fill=day)) + 
  geom_boxplot() +
  theme_classic() +
  scale_fill_manual(values = c("#9381e3", "#dff77c")) +
  xlab("Mouse Strain") +
  ylab("Total Revolutions") +
  ggtitle("Mouse Wheel Running") 
```

It turns out all the strains increase their overall running in wheels from day 1 to day 4, except the S129 strain who get bored with wheel running.
