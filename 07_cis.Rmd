...
Our observed sample mean is $\overline{x}=7.79$ which is pretty close to the population mean of $\mu=7.8$.  But if we were to collect another sample, then that sample mean will be slightly different. Let's do it again:
...
This time our sample mean (our estimate) is $\overline{x}=7.87$.
...
Let's go back to our first sample of size 15 that we collected, with $\overline{x}=7.79$.  
...
What we want to do now is put a confidence interval around 7.79. We want to say that our population mean is equal to $7.79 \pm margin.of.error$  The actual formula for the $z$-distribution confidence interval is:

$\Large CI_{95\%} = \overline{x} \pm z \times \frac{\sigma}{\sqrt{n}}$

In this scenario, we are presuming that we don't know what the population mean $\mu$ is - that's why we're building a confidence interval. Consequently, we also don't precisely know what the mean of the sampling distribution $\mu_{\overline{x}}$ is.  What we'll do instead, is to *assume* that our sample mean $\overline{x}$ is the mean of the sampling distribution $\mu_{\overline{x}}$.  We already know what the standard deviation of the sampling distribution $\sigma{\overline{x}}$ is because we know the population standard deviation $\sigma$ is.  So, $\Large \sigma_{\overline{x}} = \frac{\sigma}{\sqrt{n}}$.
...
```{r}
x_bar <- mean(samp1)  # sample mean = 7.79
x_bar
...
We have just calculated our 95% confidence interval! It has a lower bound of 7.63cm and an upper bound of 7.94cm.  We can write this confidence interval in two ways:

$CI_{95\%} = 7.79 \pm  0.152$

$CI_{95\%} = 7.79 [7.63, 7.94]$
...
Remember we collected a second sample that had a sample mean $\overline{x}=7.87$ ?
...
$\Large CI_{95\%} = 7.87 \pm  0.152$

$\Large CI_{95\%} = 7.87 [7.72, 8.02]$
...
In fact, our very first sample we obtained, `samp1`, has a relatively small sample standard deviation:
...
If we calculate the 95% CIs using a $z$- and $t$- distribution for this sample, we actually see that the $t$-distribution one is smaller.

```{r}
# 95% t distribution CI
7.79 + (2.144787 * (sd(samp1)/sqrt(15)) ) 
7.79 - (2.144787 * (sd(samp1)/sqrt(15)) )
```

```{r}
# 95% z distribution CI
7.79 + (1.96 * sem)
7.79 - (1.96 * sem)
```
